...
Thread::Thread(char* threadName, int threadID, int initP)
{
	ID = threadID;
    name = threadName;
    stackTop = NULL;
...
Thread::CheckOverflow()
{
	...
	if(stack[StackSize - 1] != STACK_FENCEPOST)
        ExceptionHandler(MemoryLimitException);
    //ASSERT(stack[StackSize - 1] == STACK_FENCEPOST);
#else
    if(*stack != STACK_FENCEPOST)
        ExceptionHandler(MemoryLimitException);
...
Thread::Yield ()
{
    ...
    // [ADD MP3] update running ticks
    this->UpdateRunningTicks();
    // [END-of-ADD]
...
Thread::Sleep (bool finishing)
{
    ...
    // [ADD MP3]
    if(!finishing){
        // not finishing, implies running -> waiting
        // Add to waiting list
        kernel->scheduler->WaitAppend(this);
        // update remaining burst time, then reset running tick
        UpdateRunningTicks();
        UpdateRemainBurst();
        // ResetRunningTicks();
        // start waiting, reset waiting ticks and start waiting tick
        ResetWaitingTicks();
        ResetStartWaitingTick();
    }
    // [END-of-ADD]
...
//----------------------------------------------------------------------
//  [ADD MP3]
//  Thread::UpdateRemainBurst
//	Update remainBurst according to totalTicks in stats, startRunTick, remainBurst.
//
//  You should run this function only once a thread leave running state
//----------------------------------------------------------------------

void
Thread::UpdateRemainBurst(){
    double newBurstTime = 0.5*T + 0.5*burstTime;
    DEBUG(dbgQLevel, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << ID << "] update approximate burst time, from: [" << burstTime << "], add [" << T << "] to [" << newBurstTime << "]");
    burstTime = newBurstTime;
}

//----------------------------------------------------------------------
//  [ADD MP3]
//  Thread::UpdateRunningTicks
//	Update running tick according to total ticks and start running tick
//----------------------------------------------------------------------

void
Thread::UpdateRunningTicks() {
    T += kernel->stats->totalTicks - startRunningTick;
}

//----------------------------------------------------------------------
//  [ADD MP3]
//  Thread::UpdateWaitingTicks
//	Update waiting tick according to total ticks and start waiting tick
//----------------------------------------------------------------------

void
Thread::UpdateWaitingTicks() {
    W += kernel->stats->totalTicks - startWaitingTick;
}

//----------------------------------------------------------------------
//  [ADD MP3]
//  Thread::ResetStartRunningTick
//	Reset start running ticks to current tick
//----------------------------------------------------------------------
void
Thread::ResetStartRunningTick() {
...
Thread::SelfTest()
{
    ...
    Thread *t = new Thread("forked thread", 1, 0); // [ADD MP3] give arbitrary inital priority now
