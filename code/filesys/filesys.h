...
//  The OpenAFile function is used for kernel open system call
    OpenFileId OpenAFile(char *name) {
        ...
        // fail to open file
        if(fd < 0) return -1;
        this->OpenFileTable[idx] = new OpenFile(fd);
        return idx;
    }
    int WriteFile(char *buffer, int size, OpenFileId id){
        // handle invalid file id
        if(id < 0 || id >= 20) return -1;
        if(this->OpenFileTable[id] == NULL) return -1;
        return this->OpenFileTable[id]->Write(buffer, size);
    }
    int ReadFile(char *buffer, int size, OpenFileId id){
        // handle invalid file id
        if(id < 0 || id >= 20) return -1;
        if(this->OpenFileTable[id] == NULL) return -1;
        return this->OpenFileTable[id]->Read(buffer, size);
    }
    int CloseFile(OpenFileId id){
        // handle invalid file id
        if(id < 0 || id >= 20) return -1;
        if(this->OpenFileTable[id] == NULL) return -1;
        free(this->OpenFileTable[id]);
        this->OpenFileTable[id] = NULL;
        return 1;
    }
